...
2.	在控制面板上，选择 Create VPC (创建 VPC)。 
 ![图片2](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/vpc-1.png)
3.	选择第一个选项，即 VPC with a Single Public Subnet，然后选择‘选择’。 
 ![图片3](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/vpc-2.png)
...
2.	在导航窗格中，选择 Internet Gateways (Internet 网关)，然后选择 Create internet gateway (创建 Internet 网关)。 
 ![图片4](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ing-1.png)
3.	选择刚刚创建的 Internet 网关，然后选择 Actions, Attach to VPC
 ![图片5](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ing-2.png)
4.	从列表中选择 VPC，然后选择 Attach (附加)。 
 ![图片6](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ing-3.png)
#### 路由表
将Internet Gateway Attach到VPC上，路由表配置0.0.0.0/0指向Internet Gateway，并关联子网。
 ![图片7](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/rtr.png)
之后的EC2宿主机集群，负载均衡器等都使用在这个网络里。
#### 负载均衡器
1. 创建ALB应用负载均衡器，监听80端口
  ![图片8](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/alb-1.png)
![图片9](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/alb-2.png)
2. 选择对应的子网
![图片10](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/alb-3.png)
#### 新建安全组，端口80，  
![图片11](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/sg.png)
#### 新建目标组
 ![图片12](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/tg.png)
...
在ECS的界面下，创建集群
 ...
 ![图片13](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-1.png)
  ![图片14](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-2.png)
实例配置保持默认或根据情况自行选择。
联网配置，选择创建好的VPC，子网，创建Role允许宿主机上的ECS代理调用ECS服务的API。
  ![图片15](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-3.png)
  ![图片16](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-4.png)
  ![图片17](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-5.png)
创建后画面下面会显示集群信息
  ![图片18](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-6.png)
集群一览会显示
  ![图片19](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-7.png)
修改ECS宿主机集群的安全组，inbound源设置为建好的应用负载均衡器的安全组ID
  ![图片20](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-8.png)
##### ECR镜像仓库
创建一个用于Build阶段上传存放软件工程Docker镜像的镜像仓库
  ![图片21](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-9.png)
ECS界面下，创建存储库，创建好后如下
  ![图片22](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/ecs-10.png)
#### S3桶创建
创建一个S3桶用来存放Deploy阶段CloudFormation使用的脚本模版，创建桶时选择和以上服务同一Region，并且打开桶的版本控制。
  ![图片23](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/s3.png)
...
5. AWS CodePipeline创建后的展示图是这样的，串起了整个CICD流程
  ![图片24](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-1.png)
在AWS CodePipeline界面点击创建管道Pipeline，可以看到画面左侧一个基本流程，从源，到build阶段生成jar，到build阶段生成docker到部署Deploy。实际应用中用户可以随实际需要，或随着CICD流程的由简入繁在创建后编辑加入新的阶段或操作。
   ![图片25](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-2.png)
 
点击下一步。
  ![图片26](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-3.png)
#### Source阶段配置
...
AWS CodePipeline在Build阶段支持包括AWS CodeBuild，Jenkins在内的引擎，此方案选用AWS 托管的CodeBuild服务
   ![图片27](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-4.png)
选择新建构建项目
   ![图片28](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-5.png)
选择AWS CodeBuild托管的镜像，支持Ubuntu系统，运行时支持包括Java，Python，Go语言，Node.js，Docker在内的众多选择，此次方案使用Java。
   ![图片29](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-6.png)
构建规范这里选择使用buildspec.yml，里面预定了AWS CodeBuild在Build生命周期中要执行的动作，如 
  ![图片30](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-7.png)
Buildspec.yml放在GitHub软件工程源代码目录中

选择Role
   ![图片31](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-8.png)
...
保存构建项目。 
    ![图片32](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-9.png)
   ![图片33](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-10.png)
打开codebuild编辑已有项目
选择构建放置的位置  类型选择S3 填入相应的存储桶名称
![图片](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/adds3.png)
点击下一步。
...
AWS CodePipeline部署阶段支持包括AWS CloudFormation，AWS CodeDeploy，AWS Elastic Beanstalk在内的服务提供商，此方案选用AWS CloudFormation来部署ECS容器服务。
    ![图片34](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-11.png)
...
配置AWS CodePipeline对AWS服务的调用权限，包括S3，AWS CodeBuild，AWS CloudFormation，IAM等。点击创建角色到IAM界面选择相对应的策略创建。
    ![图片34](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-12.png)
创建好后画面回到Pipeline，IAM创建好的Role已经显示在里面。
    ![图片35](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-13.png)
点击下一步。
#### 审核后创建管道
管道创建好后会自动运行，现有的从GitHub软件工程源代码抓取工程，打包Docker镜像并推送到ECR上。
#### 添加Deploy阶段CloudFormation需要的模版源
点击编辑，点击Source阶段右上角的画笔图标
    ![图片36](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-14.png)
可以看到AWS CodePipeline的编辑界面在南北纵向和东西横向都可以添加
     ![图片37](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-15.png)
在GitHub这个Source右侧，点击添加操作，选择源，操作名称Template，选择S3，输入创建好的S3桶的地址
     ![图片38](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-16.png)
画面往下拉，注意在输出项目这里，输入Template。
Pipeline中各阶段的传递需要制定南北向的输入输出，即Source阶段S3源的输出Template，在Deploy阶段用输入Template来衔接。
 ![图片39](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-17.png)
点击更新。
#### Build的构建dockers
点击Build阶段下面的添加阶段，画面右侧选择构建
选择新建构建项目
  ![图片40](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-18.png)
选择AWS CodeBuild托管的镜像，支持Ubuntu系统，运行时支持包括Java，Python，Go语言，Node.js，Docker在内的众多选择，此次方案使用Docker。
   ![图片41](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-19.png)
构建规范这里选择使用构建命令，里面预定了AWS CodeBuild在Build生命周期中要执行的动作，如
   ![图片42](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-20.png)
复制粘贴命令到构建命令中并修改相应参数
```
version: 0.2
phases:
...
```
选择Role
   ![图片43](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-21.png)
新建一个Role，这个Role允许AWS CodeBuild来调用相关的AWS服务，此方案中需要调用包括S3，ECR，CloudWatch在内的服务。
*默认创建的Role不具备对ECR的权限，需要在保存构建项目后，到IAM找到新创建的Role，编辑添加对ECR的权限否则后面Pipeline执行到Build时会报错。
保存构建项目。 
    ![图片44](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-22.png)
    ![图片45](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-23.png)
    ![图片46](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-24.png)
点击下一步。


#### Cloudformation
点击Build阶段下面的添加阶段，画面右侧选择部署，选择AWS CloudFormation
    ![图片47](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-25.png)
操作模式选择创建或更新堆栈，输入创建的堆栈名称，模版这里输入Template::templates/service.yaml，也就是对应的输入是S3源桶中templates.zip里的service.yaml文件。功能选择CAPABILITY_NAMED_IAM。
  ![图片47](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-26.png)
同样需要创建一个Role，允许AWS CloudFormation调用包括IAM，ECS，ECR在内的AWS服务。
  ![图片48](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-27.png)
在IAM界面创建好后选择Role。
高级这里点开
  ![图片49](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-28.png)
...
输入项目这里输入Build阶段和S3模版源的输出。
     ![图片50](https://s3.cn-north-1.amazonaws.com.cn/chinalabs/assets/CICD/pipe-29.png)
点击添加操作。
保存管道更改。
