...
2.	在导航窗格中，选择 Internet Gateways (Internet 网关)，然后选择 Create internet gateway (创建 Internet 网关)。 
 ![图片4](./assets/CICD/ing-1.png)
3.	选择刚刚创建的 Internet 网关，然后选择 Actions, Attach to VPC
 ![图片5](./assets/CICD/ing-2.png)
4.	从列表中选择 VPC，然后选择 Attach (附加)。 
 ![图片6](./assets/CICD/ing-3.png)
#### 路由表
将Internet Gateway Attach到VPC上，路由表配置0.0.0.0/0指向Internet Gateway，并关联子网。
 ![图片7](./assets/CICD/rtr.png)
之后的EC2宿主机集群，负载均衡器等都使用在这个网络里。
#### 负载均衡器
1. 创建ALB应用负载均衡器，监听80端口
  ![图片8](./assets/CICD/alb-1.png)
![图片9](./assets/CICD/alb-2.png)
2. 选择对应的子网
![图片10](./assets/CICD/alb-3.png)
#### 新建安全组，端口80，  
![图片11](./assets/CICD/sg.png)
#### 新建目标组
 ![图片12](./assets/CICD/tg.png)
...
在ECS的界面下，创建集群
 ...
 ![图片13](./assets/CICD/ecs-1.png)
  ![图片14](./assets/CICD/ecs-2.png)
实例配置保持默认或根据情况自行选择。
联网配置，选择创建好的VPC，子网，创建Role允许宿主机上的ECS代理调用ECS服务的API。
  ![图片15](./assets/CICD/ecs-3.png)
  ![图片16](./assets/CICD/ecs-4.png)
  ![图片17](./assets/CICD/ecs-5.png)
创建后画面下面会显示集群信息
  ![图片18](./assets/CICD/ecs-6.png)
集群一览会显示
  ![图片19](./assets/CICD/ecs-7.png)
修改ECS宿主机集群的安全组，inbound源设置为建好的应用负载均衡器的安全组ID
  ![图片20](./assets/CICD/ecs-8.png)
##### ECR镜像仓库
创建一个用于Build阶段上传存放软件工程Docker镜像的镜像仓库
  ![图片21](./assets/CICD/ecs-9.png)
ECS界面下，创建存储库，创建好后如下
  ![图片22](./assets/CICD/ecs-10.png)
#### S3桶创建
创建一个S3桶用来存放Deploy阶段CloudFormation使用的脚本模版，创建桶时选择和以上服务同一Region，并且打开桶的版本控制。
  ![图片23](./assets/CICD/s3.png)
...
在AWS CodePipeline界面点击创建管道Pipeline，可以看到画面左侧一个基本流程，从源，到build阶段生成jar，到build阶段生成docker到部署Deploy。实际应用中用户可以随实际需要，或随着CICD流程的由简入繁在创建后编辑加入新的阶段或操作。
   ![图片25](./assets/CICD/pipe-2.png)
 
点击下一步。
  ![图片26](./assets/CICD/pipe-3.png)
#### Source阶段配置
...
保存构建项目。 
    ![图片32](./assets/CICD/pipe-9.png)
   ![图片33](./assets/CICD/pipe-10.png)
打开codebuild编辑已有项目
选择构建放置的位置  类型选择S3 填入相应的存储桶名称
![图片](./assets/CICD/adds3.png)
点击下一步。
...
```
选择Role
   ![图片43](./assets/CICD/pipe-21.png)
新建一个Role，这个Role允许AWS CodeBuild来调用相关的AWS服务，此方案中需要调用包括S3，ECR，CloudWatch在内的服务。
*默认创建的Role不具备对ECR的权限，需要在保存构建项目后，到IAM找到新创建的Role，编辑添加对ECR的权限否则后面Pipeline执行到Build时会报错。
保存构建项目。 
    ![图片44](./assets/CICD/pipe-22.png)
    ![图片45](./assets/CICD/pipe-23.png)
    ![图片46](./assets/CICD/pipe-24.png)
点击下一步。


#### Cloudformation
点击Build阶段下面的添加阶段，画面右侧选择部署，选择AWS CloudFormation
    ![图片47](./assets/CICD/pipe-25.png)
操作模式选择创建或更新堆栈，输入创建的堆栈名称，模版这里输入Template::templates/service.yaml，也就是对应的输入是S3源桶中templates.zip里的service.yaml文件。功能选择CAPABILITY_NAMED_IAM。
  ![图片47](./assets/CICD/pipe-26.png)
同样需要创建一个Role，允许AWS CloudFormation调用包括IAM，ECS，ECR在内的AWS服务。
  ![图片48](./assets/CICD/pipe-27.png)
在IAM界面创建好后选择Role。
高级这里点开
  ![图片49](./assets/CICD/pipe-28.png)
